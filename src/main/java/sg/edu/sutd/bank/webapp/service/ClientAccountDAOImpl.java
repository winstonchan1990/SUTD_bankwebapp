...
public class ClientAccountDAOImpl extends AbstractDAOImpl implements ClientAccountDAO {
	...
	@Override
	public double getAccountBalance(ClientAccount clientAccount) throws ServiceException {
		Connection conn = connectDB();
		PreparedStatement ps = null;
		ResultSet rs = null;
		...
		try {
			ps = conn.prepareStatement(
					"SELECT amount FROM client_account WHERE user_id = ?");
			int idx = 1;
			ps.setInt(idx++, clientAccount.getUser().getId());
			rs = ps.executeQuery();
			
			while(rs.next()) {
				balanceAmt = rs.getBigDecimal("amount").doubleValue();
			}
