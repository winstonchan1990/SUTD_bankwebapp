...
public class StaffDashboardServlet extends DefaultServlet {
	...
	private BigDecimal[] toBigDecimalArray(String[] idStrs) {
		BigDecimal[] result = new BigDecimal[idStrs.length];
		for (int i = 0; i < idStrs.length; i++) {
			result[i] = BigDecimal.valueOf(Double.parseDouble(idStrs[i]));
		}
		return result;
	}
	...
	private void onTransactionDecisionAction(HttpServletRequest req, HttpServletResponse resp)
		...
		if (!transactions.isEmpty()) {
			try {
				...
				for(ClientTransaction txn : transactions) {
					if(txn.getStatus() == TransactionStatus.APPROVED) {
						ClientAccount clientAccount = new ClientAccount();
						clientAccount.setUser(txn.getUser());
						
						double oldBalance = clientAccountDAO.getAccountBalance(clientAccount);
						double txnAmt = txn.getAmount().doubleValue();
						...
						if(txnAmt < oldBalance) {
							...
							clientTransactionDAO.updateDecision(txn);
						}
						else {
							throw ServiceException.wrap(new Exception("Insufficent balance in client account."));
						}
					}
					else {
						clientTransactionDAO.updateDecision(txn);
				...
				if (e.getMessage().contains("Insufficent balance in client account.")) {
